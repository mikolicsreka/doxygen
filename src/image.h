/******************************************************************************
 *
 * 
 *
 *
 * Copyright (C) 1997-2015 by Dimitri van Heesch.
 *
 * Permission to use, copy, modify, and distribute this software and its
 * documentation under the terms of the GNU General Public License is hereby 
 * granted. No representations are made about the suitability of this software 
 * for any purpose. It is provided "as is" without express or implied warranty.
 * See the GNU General Public License for more details.
 *
 * Documents produced by Doxygen are derivative works derived from the
 * input used in their production; they are not affected by this license.
 *
 */

#ifndef _IMAGE_H
#define _IMAGE_H
#include <qglobal.h>
#include "strongtype.hpp"

enum class ImageSize { width, height, area };
enum class CharSize { width, height };
// Use of enum class ensures each StrongType is unique


using Width   = qip::StrongType<ImageSize, ImageSize::width,  uint>;
using Height     = qip::StrongType<ImageSize, ImageSize::height,  uint>;
using Area = qip::StrongType<ImageSize, ImageSize::area, uint>;

using CharWidth = qip::StrongType<CharSize, CharSize::width, const int>;
using CharHeight = qip::StrongType<CharSize, CharSize::height, const int>;


constexpr Area operator*(Width lhs, const Height& rhs) {
  return Area{ lhs.as_base() *= rhs.as_base() };
}


/** Class representing a bitmap image generated by doxygen. */
class Image
{
  public:
    Image(Width w, Height h);
   ~Image();
   
    void setPixel(uint x,uint y,uchar val);
    uchar getPixel(uint x,uint y) const;
    void writeChar(uint x,uint y,char c,uchar fg);
    void writeString(uint x,uint y,const char *s,uchar fg);
    void drawHorzLine(uint y,uint xs,uint xe,uchar colIndex,uint mask);
    void drawHorzArrow(uint y,uint xs,uint xe,uchar colIndex,uint mask);
    void drawVertLine(uint x,uint ys,uint ye,uchar colIndex,uint mask);
    void drawVertArrow(uint x,uint ys,uint ye,uchar colIndex,uint mask);
    void drawRect(uint x,uint y, Width width, Height height,uchar colIndex,uint mask);
    void fillRect(uint x,uint y,Width width, Height height,uchar colIndex,uint mask);
    bool save(const char *fileName,int mode=0);
    friend uint stringLength(const char *s);
    Width width() const { return Width {m_width}; }
    Height height() const { return Height {m_height}; }
    uchar *data() const { return  m_data; }
    static uint stringLength(const char *s);
 
  private:
    Width m_width{ 0 };
    Height m_height{ 0 };
    uchar *m_data;
};

/** Class representing a bitmap image colored based on hue/sat/gamma settings. */
class ColoredImage
{
  public:
    ColoredImage(Width width, Height height,
           const uchar *greyLevels,const uchar *alphaLevels,
           int saturation,int hue,int gamma);
   ~ColoredImage();
    bool save(const char *fileName);
    static void hsl2rgb(double h,double s,double l,
                        double *pRed,double *pGreen,double *pBlue);
  private:
    Width m_width{ 0 };
    Height m_height{ 0 };
    uchar* m_data;
    bool m_hasAlpha;
};

#endif
